...
export function middleware(request: NextRequest) {
  ...
  if (!path.startsWith('/support-panel') && !path.startsWith('/user-panel') && !path.startsWith('/org') && !path.startsWith('/agency')) return NextResponse.next();
...
export const config = { matcher: ['/support-panel/:path*', '/user-panel/:path*', '/agency/:path*'] };
